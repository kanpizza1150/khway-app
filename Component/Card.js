...
import { CircleButton } from "../Component/Button"
...
export const ContactInfo = ({ data }) => {
  return (
    <Fragment>
      ...
      />
      <InfoWithIcon value={data?.contractInfo?.socialMedia?.lineId} icon={<FontistoIcons name="line" size={20} />} />
      <InfoWithIcon
  ...
  return (
    <Fragment>
      {data.rewards.map((reward) => (
        <InfoWithIcon key={reward} value={reward} icon={<FontAwesomeIcons name="trophy" size={20} color="#F5BD02" />} />
      ))}
    </Fragment>
  )
}
